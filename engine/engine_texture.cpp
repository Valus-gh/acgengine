 ...
 */
ENG_API Eng::Texture::Texture(const std::string& name) : Eng::Object(name),
                                                         reserved(std::make_unique<Eng::Texture::Reserved>())
{
	ENG_LOG_DETAIL("[+]");
}
 ...
 * Move constructor. 
 */
ENG_API Eng::Texture::Texture(Texture&& other) : Eng::Object(std::move(other)), Eng::Managed(std::move(other)),
...
bool ENG_API Eng::Texture::load(const Eng::Bitmap& bitmap)
{
	...
	for (uint32_t side = 0; side < bitmap.getNrOfSides(); side++)
		...
		{
			ENG_LOG_DEBUG("Type: 2D, Level: %d/%d, IntFormat: 0x%x, x: %u, y: %u", c + 1, bitmap.getNrOfLevels(),
			              intFormat, bitmap.getSizeX(c), bitmap.getSizeY(c));
			switch (_format)
			{
			// Compressed:
			...
			case Format::r8_compressed:
				glCompressedTexImage2D(GL_TEXTURE_2D, c, intFormat, bitmap.getSizeX(c), bitmap.getSizeY(c), 0,
				                       bitmap.getNrOfBytes(c), bitmap.getData(c));
				break;

			// Uncompressed:
			default:
				glTexImage2D(GL_TEXTURE_2D, c, intFormat, bitmap.getSizeX(c), bitmap.getSizeY(c), 0, extFormat, extType,
				             bitmap.getData(c));
			}
		}
