 ...
 * Constructor.
 */
ENG_API Eng::Bitmap::Bitmap(Format format, uint32_t sizeX, uint32_t sizeY, uint8_t* data) : reserved(
	std::make_unique<Eng::Bitmap::Reserved>())
{
	ENG_LOG_DETAIL("[+]");
 ...
 */
ENG_API Eng::Bitmap::Bitmap(const std::string& name) : Eng::Object(name),
                                                       reserved(std::make_unique<Eng::Bitmap::Reserved>())
{
	ENG_LOG_DETAIL("[+]");
}
...
bool ENG_API Eng::Bitmap::load(Format format, uint32_t sizeX, uint32_t sizeY, uint8_t* data)
{
	...
	switch (format)
	{
	case Format::r8g8b8: colorDepth = 3;
		break;
	case Format::r8g8b8a8: colorDepth = 4;
		break;
	default:
...
bool ENG_API Eng::Bitmap::load(const std::string& filename)
{
	...
	memcpy(&magicNumber, position, sizeof(uint32_t));
	...
	// Get header:
	DDS_HEADER* header = reinterpret_cast<DDS_HEADER*>(position);
	position += sizeof(DDS_HEADER);
	...
	switch (reserved->format)
	{
	case Eng::Bitmap::Format::r8_compressed: reserved->compressionFactor = 0.5f;
		break;
	case Eng::Bitmap::Format::r8g8_compressed: reserved->compressionFactor = 1.0f;
		break;
	case Eng::Bitmap::Format::r8g8b8_compressed: reserved->compressionFactor = 0.5f;
		break;
	case Eng::Bitmap::Format::r8g8b8a8_compressed: reserved->compressionFactor = 1.0f;
		break;
	}
	...
	for (uint32_t s = 0; s < reserved->nrOfSides; s++)
	{
		...
		for (uint32_t c = 0; c < reserved->nrOfLevels; c++)
		{
			...
			memcpy(curLayer.data.data(), position, levelSize);
			position += levelSize;
